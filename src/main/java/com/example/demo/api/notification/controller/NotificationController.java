...
public class NotificationController {
    ...
    public ApiResponseDto<NotificationResponse> sendNotification(
        ...
        Notification notification = notificationUseCase.sendNotification(userId, request);
    ...
    public ApiResponseDto<NotificationPageResponse> getNotification(
        ...
        Page<Notification> notificationPage = notificationUseCase.getNotificationList(userId, pageable);
    ...
    public ApiResponseDto<Long> updateNotification(@PathVariable Long userId, @PathVariable Long notificationId) {
        return ApiResponseDto.onSuccess(notificationUseCase.readNotification(userId, notificationId));
    }

    // TODO: @AuthUser 적용하기
    @Operation(summary = "user의 notification을 삭제", description = "user의 notification을 삭제합니다.")
    @DeleteMapping("/{notificationId}")
    public ApiResponseDto<Long> deleteNotification(@PathVariable Long userId, @PathVariable Long notificationId) {
        return ApiResponseDto.onSuccess(notificationUseCase.deleteNotification(userId, notificationId));
    }
}
