...
public class NotificationController {
    ...
    @GetMapping
    public ApiResponseDto<NotificationResponseDto.NotificationPageResponse> getNotification(
            User user,
            @RequestParam(defaultValue = "0") int page,
            @RequestParam(defaultValue = "10") int pageSize) {
        Pageable pageable = PageRequest.of(page, pageSize, Sort.by(Sort.Order.desc("createdDate")));
        Page<Notification> notificationPage = notificationQueryService
                .getNotificationListByPage(user.getId(), pageable);
    ...
    public ApiResponseDto<Long> updateNotification(@PathVariable Long notificationId) {
        ...
        return ApiResponseDto.onSuccess(notificationCommandService.readNotification(notificationId));
    }
