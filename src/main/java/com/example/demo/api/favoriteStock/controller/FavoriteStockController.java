...
public class FavoriteStockController {
    FavoriteStockCommandService favoriteStockCommandService;
    FavoriteStockUseCase  favoriteStockUseCase;
    ...
    public ApiResponseDto<Long> addUserFavoriteStock(User user,
        ...
        return ApiResponseDto.onSuccess(favoriteStockCommandService.addFavoriteStock(favoriteStock));
    }

    // TODO: Member와 User 통합 후, @AuthUser 만들고 적용
    @Operation(summary = "user의 관심 종목들 목록", description = "user의 관심 종목들을 pagenation으로 조회합니다.")
    @GetMapping
    public ApiResponseDto<FavoriteStockSimplePageResponse> getFavoriteStockByPage(
            User user,
    ...
    public ApiResponseDto<Void> deleteUserFavoriteStock(@PathVariable Long favoriteStockId) {
        return ApiResponseDto.onSuccess(favoriteStockCommandService.deleteFavoriteStock(favoriteStockId));
    }
}
